  ...
  return (
    <section className="mx-auto max-w-5xl px-4 sm:px-6 py-12 sm:py-24">
      <div className="grid gap-8 sm:gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div className="space-y-4 sm:space-y-6 rounded-2xl sm:rounded-[2rem] border border-neutral-800/80 bg-neutral-900/80 p-6 sm:p-12 shadow-2xl shadow-black/20">
          <span className="text-xs sm:text-sm uppercase tracking-[0.3em] text-amber-400 font-bold">Join the experience</span>
          <h1 className="text-3xl sm:text-4xl font-serif text-white">Create your account and reserve an unforgettable evening.</h1>
          <p className="max-w-xl text-xs sm:text-sm text-neutral-400 leading-6 sm:leading-7">
            Register once to save your details, access exclusive invitations, and receive seasonal menu previews from our dining house.
          </p>
        </div>

        <form onSubmit={handleAuth} className="space-y-4 sm:space-y-6 rounded-2xl sm:rounded-[2rem] border border-neutral-800/80 bg-neutral-950/85 p-6 sm:p-10 shadow-xl shadow-black/20">
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-wide text-neutral-400 font-semibold">Full name</label>
            <input type="text" required onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full rounded-2xl border border-neutral-800 bg-neutral-900 px-4 py-3 text-sm text-white outline-none focus:border-amber-500" />
          </div>
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-wide text-neutral-400 font-semibold">Email address</label>
            <input type="email" required onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full rounded-2xl border border-neutral-800 bg-neutral-900 px-4 py-3 text-sm text-white outline-none focus:border-amber-500" />
          </div>
          <div className="space-y-2">
